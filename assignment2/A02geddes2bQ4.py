###############################################################################################
# -
# - Title:          A02geddes2bQ4.py
# - Author:         Brant Geddes
# - Date:           Febuary 2020
# -
# - Description:    Implements Softmax Regression on the MNIST dataset to classify images into 
# -                 digits ranging from 0-9, uses l2 regularization in the optimization
    ...
    def fit(self, _Y, Y, X):
        ...
        for index in range(len(_Y)):
            for i in range(self.classes):
                # Regularization term (C/n * W)
                reg = 2 * (self.C/self.datapoints) * self.W.T[i].reshape(1, self.features)
                # Bias update
                self.b = self.b - self.learning * 2 * (_Y[index][i] - Y[index][i])
                # Weight update
                _W[i] = self.normalize(_W[i] - self.learning * (2 * (_Y[index][i] - Y[index][i]) * X[index] + reg))
        self.W = _W.T
...
while counter < model.epochs:
    ...
    # Make prediction for training metrics and calculate training loss and accuracy
    prediction = model.predict(model.data[:model.batch])
    ...
    # Make prediction for training metrics and calculate testing loss and accuracy
...
try:
    ...
    plt.xlabel('Epochs')
    plt.title('MNIST Logistic Regression (Q4)')
